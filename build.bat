...
pscp "%VERSIONED_ZIP%" "%REMOTE_HOST%:%REMOTE_DIR%/"
if %ERRORLEVEL% neq 0 ( echo ERROR: Upload of ZIP failed. & exit /b 1 )
pscp "%VERSIONED_INI%" "%REMOTE_HOST%:%REMOTE_DIR%/"
if %ERRORLEVEL% neq 0 ( echo ERROR: Upload of INI failed. & exit /b 1 )
plink %REMOTE_HOST% "cd %REMOTE_DIR% && ln -sf %VERSIONED_ZIP% noncey-chromeext.zip && ln -sf %VERSIONED_INI% noncey-chromeext.ini"
